...
int InitServices()
{
	...
	commonData.lPopupClick = DBGetContactSettingByte(NULL, ModuleName, "PopupLeftClick", 0);
	...
	commonData.notifyFor = DBGetContactSettingByte(NULL, ModuleName, "NotifyFor", 3);	
...
INT_PTR CheckBirthdaysService(WPARAM wParam, LPARAM lParam)
{
	...
	if (daysAfter > commonData.daysAfter) //check for passed birthdays
	{
