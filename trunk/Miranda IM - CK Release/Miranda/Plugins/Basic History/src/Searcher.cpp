...
void Searcher::Find()
{
	...
	if(lastFindSelection >= 0 && lastFindSelection < context.currentGroup.size())
	{
		...
		else
		{
			...
			{
				if(isFindSelChanged && context.selected == startFindSel && ((!findBack1 && ft.chrg.cpMin >= startFindPos) || (findBack1 && ft.chrg.cpMax <= startFindPos)))
				{
					finished = true;
				}
				else
	...
	else
	{
		...
		if(findBack1)
			for(curSel = context.currentGroup.size() - 1; curSel >= 0; --curSel)
			{
				if(context.currentGroup[curSel].startPos < startFindPos)
					break;
			}
		else
			for(; curSel < context.currentGroup.size(); ++curSel)
			{
	...
	if(!finished)
	{
		for(; curSel < context.currentGroup.size() && curSel >= 0; curSel += adder1)
		{
	...
	if(isFindSelChanged && context.selected == startFindSel)
	{
		finished = true;
	}
	...
	if(!finished)
	{
		...
		if(onlyGroup)
		{
			if(IsInSel(context.selected, str))
			{
				...
				lastFindSelection = findBack1 ? context.currentGroup.size() - 1 : 0;
				Find();
				return;
			}
		}
		else
		{
			...
			{
				if(sel < 0)
					sel = context.eventList.size() - 1;
				else if(sel >= context.eventList.size())
					sel = 0;
				if(IsInSel(sel, str))
				{
					LVITEM item = {0};
