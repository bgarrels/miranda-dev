...
%ZIP% a -mx9 upload\updater\tipper-ym.zip tipper.dll docs\*.txt docs\*.ini docs\*.h skins

COPY /Y "Release (ansi)\tipper.dll" tipper.dll
%ZIP% a -mx9 upload\updater\tipper-ym_ansi.zip tipper.dll docs\*.txt docs\*.ini docs\*.h
%ZIP% a -mx9 upload\tipper-ym_src.zip *.cpp *.h *.rc *.vcproj *.ds* docs\*.txt docs\*.ini docs\*.h sdk\*.h res\*.ico

COPY /Y upload\updater\tipper-ym.zip upload\addons\tipperymw.zip
COPY /Y upload\updater\tipper-ym_ansi.zip upload\addons\tipperyma.zip
