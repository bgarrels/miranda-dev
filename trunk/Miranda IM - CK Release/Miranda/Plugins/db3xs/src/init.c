...
{
	strncpy(buf, shortName ? "db3xS driver" : "db3xS database support", cch);
	return 0;
}
...
{
	if ( mirandaVersion < PLUGIN_MAKE_VERSION(0,9,1,0)) {
		MessageBox( NULL, _T("The db3xS plugin cannot be loaded. It requires Miranda IM 0.9.1 or later."), _T("db3xS Plugin"), MB_OK|MB_ICONWARNING|MB_SETFOREGROUND|MB_TOPMOST );
		return NULL;
	}
	return &pluginInfo;
}
