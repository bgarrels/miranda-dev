...
static unsigned __stdcall oft_connectionThread(void *pParam);
...
const char* FindFilePathContainer(const char** files, int iFile, char* szContainer)
{
    int i;
    const char* szThisFile = files[iFile];
    const char* szFileName = ExtractFileName(szThisFile);
...
static oscar_filetransfer* CreateOscarTransfer()
{
    oscar_filetransfer* ft = (oscar_filetransfer*)icq_alloc_zero(sizeof(oscar_filetransfer));
    ...
    fileTransferList = (basic_filetransfer**)mir_realloc(fileTransferList, sizeof(basic_filetransfer*)*(fileTransferCount + 1));
...
{
    filetransfer *ft = (filetransfer*)icq_alloc_zero(sizeof(filetransfer));
...
static void ReleaseFileTransfer(void *ft)
{
    ...
    if (i != -1)
    {
        ...
        fileTransferList = (basic_filetransfer**)mir_realloc(fileTransferList, sizeof(basic_filetransfer*)*fileTransferCount);
    }
}
...
void SafeReleaseFileTransfer(void **ft)
{
    ...
    if (*bft)
    {
        if ((*bft)->ft_magic == FT_MAGIC_ICQ)
        {
            ...
            mir_free(ift->szFilename);
            mir_free(ift->szDescription);
            mir_free(ift->szSavePath);
            mir_free(ift->szThisFile);
            mir_free(ift->szThisSubdir);
            if (ift->pszFiles)
            {
                int i;

                for (i = 0; i < (int)ift->dwFileCount; i++)
                    mir_free(ift->pszFiles[i]);
                mir_free(ift->pszFiles);
            }
            // Invalidate transfer
        ...
        else if ((*bft)->ft_magic == FT_MAGIC_OSCAR)
        {
            ...
            if (oft->connection)
                CloseOscarConnection((oscar_connection*)oft->connection);
            ...
            mir_free(oft->rawFileName);
            mir_free(oft->szSavePath);
            mir_free(oft->szThisFile);
            mir_free(oft->szThisPath);
            mir_free(oft->szDescription);
            if (oft->files)
            {
                int i;

                for (i = 0; i < oft->wFilesCount; i++)
                    mir_free(oft->files[i].szFile);
                mir_free(oft->files);
            }
            if (oft->files_ansi)
            {
                ...
                for (i = 0; i < oft->wFilesCount; i++)
                    mir_free(oft->files_ansi[i]);
                mir_free(oft->files_ansi);
            }
            if (oft->file_containers)
            {
                ...
                for (i = 0; i < oft->containerCount; i++)
                    mir_free(oft->file_containers[i]);
                mir_free(oft->file_containers);
            }
            if (oft->fileId != -1)
            {
#ifdef _DEBUG
...
{
    oscar_listener *listener = (oscar_listener*)icq_alloc_zero(sizeof(oscar_listener));
            ...
            if (wAckType == 1)
            {
...

                {
                    ...
                    if (tlv)
                    {
                        ...
                        pszDescription = (char*)icq_alloc_zero(tlv->wLen + 2);
                        ...
                        pszDescription[tlv->wLen+1] = '\0';
                        {
                            ...
                            {
                                // decode charset
                                char *szEnc = (char*)icq_alloc_zero(charset->wLen + 1);

                                strncpy(szEnc, (char*)charset->pData, charset->wLen);	// FIXME: 2
                ...
                }
                {
                    ...
                    pszFileName = (char*)icq_alloc_zero(tLen);
                    ...
                    else
                    {
                        ...
                        pszFileName = (char*)icq_alloc_zero(64);
                ...
                }
                {
                    ...
                    szAnsi = mir_utf8decodeA(pszDescription);
                    // Send chain event
                    szBlob = (char*)icq_alloc_zero(sizeof(DWORD) + strlennull(pszFileName) + strlennull(szAnsi) + 2);
...
static char* oftGetFileContainer(oscar_filetransfer* oft, const char** files, int iFile)
{
    ...
    const char* szFileName = FindFilePathContainer(files, iFile, szPath);
    char* szPathUtf = mir_utf8encode(szPath);
    int i;
    ...
    oft->file_containers = (char**)mir_realloc(oft->file_containers, (sizeof(char*) * oft->containerCount));
...
int oftInitTransfer(HANDLE hContact, DWORD dwUin, char* szUid, char** files, char* pszDesc)
{
    ...
    ft->files = (oft_file_record *)icq_alloc_zero(sizeof(oft_file_record) * filesCount);
    ft->files_ansi = (char **)icq_alloc_zero(sizeof(char *) * filesCount);
    ft->qwTotalSize = 0;
    ...
    ft->szDescription = mir_utf8encode(pszDesc);
    ft->sending = 1;
    ft->fileId = -1;
    ft->iCurrentFile = 0;
    ...
    // Send file transfer request
    {
        ...
        if (ft->wFilesCount == 1)
        {
            // transferring single file, give filename
            pszFiles = ExtractFileName(ft->files[0].szFile);
        }
        else
        {
            // check if transferring one directory
...
DWORD oftFileAllow(HANDLE hContact, WPARAM wParam, LPARAM lParam)
{
    ...
    if((_mirandaVersion < PLUGIN_MAKE_VERSION(0, 9, 0, 1)) || (_mirandaVersion > PLUGIN_MAKE_VERSION(0, 9, 0, 6))) //this will be removed in near future, if miranda api do not change again
        ft->szSavePath = mir_utf8encode((char *)lParam);
    else
    {
        char path[MAX_PATH];
        CallService(MS_FILE_GETRECEIVEDFILESFOLDER, (WPARAM)hContact, (LPARAM)path);
        ft->szSavePath = mir_utf8encode(path);
    }
    if (ft->szThisPath)
    {
        // Append Directory name to the save path, when transfering a directory
        ft->szSavePath = (char*)mir_realloc(ft->szSavePath, strlennull(ft->szSavePath) + strlennull(ft->szThisPath) + 4);
...
void oftFileResume(oscar_filetransfer *ft, int action, const char *szFilename)
{
    ...
    case FILERESUME_RENAME:
        ...
        ft->szThisFile = mir_utf8encode(szFilename);
        ft->qwFileBytesDone = 0;
        break;
...
//  utf8_decode(ft->szPath, &pfts->workingDir); // not used by the UI anyway
    pfts->currentFile = mir_utf8decodeA(ft->szThisFile);
...
#ifndef _08CORE
static void oft_buildProtoFileTransferStatus_w(oscar_filetransfer* ft, PROTOFILETRANSFERSTATUS_W* pfts) //this need to be fixed
{
    ZeroMemory(pfts, sizeof(PROTOFILETRANSFERSTATUS_W));
    pfts->cbSize = sizeof(PROTOFILETRANSFERSTATUS_W);
    pfts->hContact = ft->hContact;
    pfts->flags = ((ft->sending) ? PFTS_SENDING : PFTS_RECEIVING);
    pfts->pszFiles = ft->files_ansi;
    pfts->totalFiles = ft->wFilesCount;
    pfts->currentFileNumber = ft->iCurrentFile;
    pfts->totalBytes = (DWORD)ft->qwTotalSize;
    pfts->totalProgress = (DWORD)ft->qwBytesDone;
    //pfts->wszWorkingDir = mir_utf8decodeW(ft->szSavePath);
    pfts->szCurrentFile = mir_utf8decodeA(ft->rawFileName);
    pfts->currentFileSize = (DWORD)ft->qwThisFileSize;
    pfts->currentFileTime = ft->dwThisFileDate;
    pfts->currentFileProgress = (DWORD)ft->qwFileBytesDone;
}
#endif
...
{
    oscarthreadstartinfo *otsi = (oscarthreadstartinfo*)icq_alloc_zero(sizeof(oscarthreadstartinfo));
...
static unsigned __stdcall oft_connectionThread(void *pParam)
{
    oscarthreadstartinfo *otsi = (oscarthreadstartinfo *)pParam;
...
static void handleOFT2FramePacket(oscar_connection *oc, WORD datatype, BYTE *pBuffer, WORD wLen)
{
    ...
    case OFT_TYPE_REQUEST:
    {
        ...
        mir_free(ft->rawFileName); // release previous buffers
        mir_free(ft->szThisFile);
        ft->rawFileName = (char*)icq_alloc_zero(ft->cbRawFileName + 2);
        ...
        {
            ft->szThisFile = mir_utf8encode(ft->rawFileName);
        }
...

        {
            ...
            mir_free(ft->szThisPath); // release previous path
            if (szFile)
            {
        ...
        szFullPath = (char*)icq_alloc_zero(strlennull(ft->szSavePath)+strlennull(ft->szThisPath)+strlennull(ft->szThisFile)+3);
...
static void oft_sendFileData(oscar_connection *oc)
{
    ...
    if (bytesRead)
    {
        ...
        packBuffer(&packet, (char*)buf, (WORD)bytesRead); // we are sending raw data
        sendOscarPacket(oc, &packet);
...
static void oft_sendPeerInit(oscar_connection *oc)
{
...

    {
        ...
        pszThisFileName = (char*)icq_alloc_zero(strlennull(ft->szThisFile) + strlennull(szThisContainer) + 4);
    ...
    if (IsUSASCII(pszThisFileName, strlennull(pszThisFileName)))
    {
        ...
        ft->rawFileName = (char*)icq_alloc_zero(ft->cbRawFileName);
    ...
    else
    {
        ...
        pwsThisFile = mir_utf8decodeW(pszThisFileName);
        mir_free(pszThisFileName);
...
static void sendOFT2FramePacket(oscar_connection *oc, WORD datatype)
{
    ...
    packBuffer(&packet, (char*)ft->rawDummy, 69);
    packBuffer(&packet, (char*)ft->rawMacInfo, 16);
