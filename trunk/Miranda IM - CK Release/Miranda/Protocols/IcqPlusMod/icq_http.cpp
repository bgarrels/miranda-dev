...
int icq_httpGatewayBegin(HANDLE hConn, NETLIBOPENCONNECTION* nloc)
{
    ...
    res = Netlib_Send(hConn, (char*)packet.pData, packet.wLen, MSG_DUMPPROXY|MSG_NOHTTPGATEWAYWRAP);
...
int icq_httpGatewayWrapSend(HANDLE hConn, PBYTE buf, int len, int flags, MIRANDASERVICE pfnNetlibSend)
{
    ...
    while (sendLen > 0)
    {
        ...
        if (sendLen > 512) curLen = 512;
        else curLen = (WORD)sendLen;
        // send wrapped data
        packet.wLen = curLen;
        write_httphdr(&packet, HTTP_PACKETTYPE_FLAP, GetGatewayIndex(hConn));
        packBuffer(&packet, (char*)sendBuf, (WORD)curLen);
        ...
        if (curResult <= 0)
            return curResult;
        // calculate real number of data bytes sent
