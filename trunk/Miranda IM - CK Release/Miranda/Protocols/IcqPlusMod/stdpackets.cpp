...
static DWORD sendDirectorySearchPacket(const BYTE *pSearchData, WORD wDataLen, WORD wPage, BOOL bOnlineUsersOnly);
...
static void packServIcqDirectoryHeader(icq_packet *p, WORD wLen, WORD wType, WORD wCommand, WORD wSeq)
{
    packServIcqExtensionHeader(p, wLen + 0x1E, CLI_META_INFO_REQ, wSeq);
    packLEWord(p, wType);
    packLEWord(p, wLen + 0x1A);
    packFNACHeaderFullwVersion(p, 0x5b9, wCommand, 0, 0, 2);
    packWord(p, 0);
...
void icq_requestnewfamily(WORD wFamily, void (*familyhandler)(HANDLE hConn, char* cookie, WORD cookieLen))
{
    ...
    request = (familyrequest_rec*)icq_alloc_zero(sizeof(familyrequest_rec));
...
void sendOwnerInfoRequest(void)
{
    ...
    fam15_cookie_data *pCookieData = (fam15_cookie_data*)icq_alloc_zero(sizeof(fam15_cookie_data));
    pCookieData->bRequestType = DIRECTORYREQUEST_INFOOWNER;

    dwCookie = AllocateCookie(CKT_DIRECTORY_QUERY, 0, NULL, (void*)pCookieData);
    wDataLen = getUINLen(dwLocalUIN) + 4;

    packServIcqDirectoryHeader(&packet, wDataLen + 8, META_DIRECTORY_QUERY, DIRECTORY_QUERY_INFO, (WORD)dwCookie);
    packWord(&packet, 0x03); // with interests (ICQ6 uses 2 at login)
    packDWord(&packet, 0x01);
    packWord(&packet, wDataLen);

    packTLVUID(&packet, 0x32, dwLocalUIN, NULL);
...
DWORD sendUserInfoMultiRequest(BYTE *pRequestData, WORD wDataLen, int nItems)
{
    ...
    packServIcqDirectoryHeader(&packet, wDataLen + 2, META_DIRECTORY_QUERY, DIRECTORY_QUERY_MULTI_INFO, (WORD)dwCookie);
    packWord(&packet, nItems);
    packBuffer(&packet, (char*)pRequestData, wDataLen);
...
DWORD icq_sendGetInfoServ(HANDLE hContact, DWORD dwUin, int bManual)
{
    icq_packet packet;
    DWORD dwCookie = 0;
    fam15_cookie_data *pCookieData = {0};
    ...
    if (!getSetting(hContact, DBSETTING_METAINFO_TOKEN, &infoToken))
    {
        ...
        pToken = (BYTE*)icq_alloc_zero(cbToken);
        memcpy(pToken, infoToken.pbVal, cbToken);

        ICQFreeVariant(&infoToken);
    }

    pCookieData = (fam15_cookie_data*)icq_alloc_zero(sizeof(fam15_cookie_data));
    ...
    wDataLen = cbToken + getUINLen(dwUin) + (cbToken ? 8 : 4);
    ...
    packWord(&packet, wDataLen);
    if (pToken)
        packTLV(&packet, 0x3C, cbToken, (char*)pToken);
...
void icq_sendFileSendServv7(filetransfer* ft, const char *szFiles)
{
    ...
    szFilesAnsi = mir_utf8decodeA(szFiles);
    wFilesLen = strlennull(szFilesAnsi);
    szDescrAnsi = mir_utf8decodeA(ft->szDescription);
    ...
    packBuffer(&packet, szDescrAnsi, (WORD)(wDescrLen + 1));
    packLEDWord(&packet, 0);   // unknown
    packLEWord(&packet, (WORD)(wFilesLen + 1));
    packBuffer(&packet, szFilesAnsi, (WORD)(wFilesLen + 1));
    packLEDWord(&packet, ft->dwTotalSize);
    packLEDWord(&packet, 0);   // unknown

    mir_free(szFilesAnsi);
    mir_free(szDescrAnsi);
...
void icq_sendFileSendServv8(filetransfer* ft, const char *szFiles, int nAckType)
{
    ...
    packBuffer(&packet, szDescrAnsi, wDescrLen);
...
void icq_sendFileAcceptServv8(DWORD dwUin, DWORD TS1, DWORD TS2, DWORD dwCookie, const char *szFiles, const char *szDescr, DWORD dwTotalSize, WORD wPort, BOOL accepted, int nAckType)
{
    ...
    szFilesAnsi = mir_utf8decodeA(szFiles);
    szDescrAnsi = mir_utf8decodeA(szDescr);
    wDescrLen = strlennull(szDescrAnsi);
    wFilesLen = strlennull(szFilesAnsi);
    ...
    if (validateStatusMessageRequest(hContact, msgType))
    {
        ...
        if (szMsg && *szMsg)
        {
            ...
            else
            {
                ...
                szAnsiMsg = mir_utf8decodeA(*szMsg);
                pszMsg = szAnsiMsg;
            }
...
DWORD SearchByNames_v9(const TCHAR *nick, const TCHAR *firstName, const TCHAR *lastName)
{
    if (icqOnline)
    {
        ...
        {
            char *nickUtf = mir_utf8encodeW(nick);
            char *firstNameUtf = mir_utf8encodeW(firstName);
            char *lastNameUtf = mir_utf8encodeW(lastName);

            // Success
            DWORD dwCookie = SearchByNames(nickUtf, firstNameUtf, lastNameUtf, 0);

            mir_free(nickUtf);
            mir_free(firstNameUtf);
            mir_free(lastNameUtf);
...
DWORD SearchByUin(DWORD dwUin)
{
    ...
    pBuffer.wPlace = 0;
    pBuffer.pData = (BYTE *)icq_alloc_zero(wInfoLen);
...
DWORD SearchByNames(const char *pszNick, const char *pszFirstName, const char *pszLastName, WORD wPage)
{
    ...
    if (wNickLen)
        packTLV(&pBuffer, 0x78, wNickLen, pszNick);

    if (wLastLen)
        packTLV(&pBuffer, 0x6E, wLastLen, pszLastName);

    if (wFirstLen)
        packTLV(&pBuffer, 0x64, wFirstLen, pszFirstName);
...
DWORD SearchByMail(char* pszEmail)
{
    ...
    if (wEmailLen > 0)
    {
        ...
        // Initialize our handy data buffer
        pBuffer = (BYTE *)icq_alloc_zero(wInfoLen);
        pBufferPos = 0;
...
DWORD sendTLVSearchPacket(BYTE bType, char* pSearchDataBuf, WORD wSearchType, WORD wInfoLen, BOOL bOnlineUsersOnly)
{
    ...
    pCookie = (search_cookie*)icq_alloc_zero(sizeof(search_cookie));
    if (!pCookie)
        return 0;
    ...
    if (wSearchType == META_SEARCH_GENERIC && bOnlineUsersOnly)
    {
        ...
        BYTE bData = 1;
        packTLV(&packet, TLV_ONLINEONLY, 1, (char*)&bData);
    }
...
DWORD sendDirectorySearchPacket(const BYTE *pSearchData, WORD wDataLen, WORD wPage, BOOL bOnlineUsersOnly)
{
    ...
    // Pack headers
    packServIcqDirectoryHeader(&packet, wDataLen + (bOnlineUsersOnly ? 14 : 8), META_DIRECTORY_QUERY, DIRECTORY_QUERY_INFO, (WORD)dwCookie);
    ...
    packBuffer(&packet, (const char*)pSearchData, wDataLen);
...
DWORD icq_sendAdvancedSearchServ(BYTE* fieldsBuffer,int bufferLen)
{
    ...
    packServIcqExtensionHeader(&packet, (WORD)bufferLen, CLI_META_INFO_REQ, (WORD)dwCookie);
...
DWORD icq_changeUserPasswordServ(const char *szPassword)
{
    ...
    packServIcqExtensionHeader(&packet, (WORD)(wPasswordLen + 4), CLI_META_INFO_REQ, (WORD)dwCookie);
    packLEWord(&packet, META_SET_PASSWORD_REQ);
    packLEWord(&packet, wPasswordLen);
    packBuffer(&packet, szPassword, wPasswordLen);
...
DWORD icq_changeUserDirectoryInfoServ(const BYTE *pData, WORD wDataLen, BYTE bRequestType)
{
    ...
    dwCookie = AllocateCookie(CKT_DIRECTORY_UPDATE, 0, NULL, pCookieData);

    packServIcqDirectoryHeader(&packet, wDataLen + 4, META_DIRECTORY_UPDATE, DIRECTORY_SET_INFO, (WORD)dwCookie);
    packWord(&packet, 0x0003);
    packWord(&packet, wDataLen);
    packBuffer(&packet, (const char*)pData, wDataLen);
...
DWORD icq_sendSMSServ(const char *szPhoneNumber, const char *szMsg)
{
    ...
    if (szBuffer = (char *)icq_alloc_zero(nBufferSize))
    {
...
void oft_sendFileRequest(DWORD dwUin, char *szUid, oscar_filetransfer* ft, const char* pszFiles, DWORD dwLocalInternalIP)
{
    ...
    szCoolStr = (char*)icq_alloc_zero(strlennull(ft->szDescription)+strlennull(pszFiles) + 160);
