...
static unsigned __stdcall icq_directThread(void* p);
...
static int CListMW_DcExtraIconsRebuild(WPARAM wParam, LPARAM lParam);
static int CListMW_DcExtraIconsApply(WPARAM wParam, LPARAM lParam);
...
void InitDCEvents(void)
{
    ...
    null_snprintf(szSection, sizeof(szSection), ICQTranslateUtfStatic("%s", str, MAX_PATH), ICQTranslateUtfStatic(ICQ_PROTOCOL_NAME, prt, MAX_PATH));
    ...
    hHookExtraIconsRebuild = HookEvent(ME_CLIST_EXTRA_LIST_REBUILD, CListMW_DcExtraIconsRebuild);
    hHookExtraIconsApply = HookEvent(ME_CLIST_EXTRA_IMAGE_APPLY, CListMW_DcExtraIconsApply);
}
...
static void ResizeDirectList(int nSize)
{
    if ((directConnSize < nSize) || ((directConnSize > nSize + 6) && nSize))
    {
        ...
        directConnList = (directconnect**)mir_realloc(directConnList, sizeof(directconnect*) * directConnSize);
    }
}
...
void AddExpectedFileRecv(filetransfer* ft)
{
    ...
    expectedFileRecv = (filetransfer** )mir_realloc(expectedFileRecv, sizeof(filetransfer *) * (expectedFileRecvCount + 1));
...
filetransfer *FindExpectedFileRecv(DWORD dwUin, DWORD dwTotalSize)
{
    ...
    for (i = 0; i < expectedFileRecvCount; i++)
    {
        if (expectedFileRecv[i]->dwUin == dwUin && expectedFileRecv[i]->dwTotalSize == dwTotalSize)
        {
            ...
            expectedFileRecv = (filetransfer**)mir_realloc(expectedFileRecv, sizeof(filetransfer*) * expectedFileRecvCount);
...
directthreadstartinfo* CreateDTSI(HANDLE hContact, HANDLE hConnection, int type)
{
    ...
    dtsi = (directthreadstartinfo*)icq_alloc_zero(sizeof(directthreadstartinfo));
...
static void setContactDcExtraIcon(HANDLE hContact, BOOL bFunc, BOOL bRem)
{
...
static int CListMW_DcExtraIconsApply(WPARAM wParam, LPARAM lParam)
{
    if (ServiceExists(MS_CLIST_EXTRA_SET_ICON))
    {
        ...
        {
            setContactDcExtraIcon((HANDLE)wParam, 0, 0);
        }
    }
    return 0;
}
...
void CloseDirectConnection(directconnect *dc)
{
    ...
    setContactDcExtraIcon(hContact2, 0, 1);
}
...
static unsigned __stdcall icq_directThread(void* p)
{
    directthreadstartinfo *dtsi = (directthreadstartinfo *)p;
    ...
    if (!dc.incoming)
    {
        ...
        if(dc.hConnection)
            setContactDcExtraIcon(dc.hContact, 1, 0);
        ...
        if (!dc.hConnection)
        {
            if (CheckContactCapabilities(dc.hContact, CAPF_DIRECT))
            {
                ...
                {
                    // try reverse connect
                    reverse_cookie *pCookie = (reverse_cookie*)icq_alloc_zero(sizeof(reverse_cookie));
                    DWORD dwCookie;
    ...
    if(!dc.hConnection) setContactDcExtraIcon(dc.hContact, 0, 1);
...
static int DecryptDirectPacket(directconnect* dc, PBYTE buf, WORD wLen)
{
...
#ifdef _DEBUG
    {
        ...
        szBuf = (char*)icq_alloc_zero(titleLineLen + ((wLen+15)>>4) * 76 + 1);
