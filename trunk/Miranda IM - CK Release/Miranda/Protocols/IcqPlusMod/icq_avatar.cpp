...
static unsigned __stdcall icq_avatarThread(void *pParam);
...
{
    avatarrequest *pNext = (avatarrequest*)request->pNext;
    ...
    if (!bAvatarsFolderInited)
    {
        ...
        if (ServiceExists(MS_FOLDERS_REGISTER_PATH))
        {
            ...
            if (_mirandaVersion >= PLUGIN_MAKE_VERSION(0, 8, 0, 29))
            {
                ...
                null_snprintf(szPath, MAX_PATH, "%s\\%s\\", tmpPath, ICQ_PROTOCOL_NAME);
                mir_free(tmpPath);
            }
            else
                null_snprintf(szPath, MAX_PATH, "%s\\%s\\", PROFILE_PATH, ICQ_PROTOCOL_NAME);

            hAvatarsFolder = FoldersRegisterCustomPath(ICQ_PROTOCOL_NAME, "Avatars Cache", szPath);
        }
    }
}
...
void GetAvatarFileName(int dwUin, char* szUid, char* pszDest, int cbLen)
{
    ...
    if (CallService(MS_FOLDERS_GET_PATH, (WPARAM)hAvatarsFolder, (LPARAM)&fgd))
    {
        ...
        tPathLen = null_snprintf(pszDest, cbLen, "%s\\%s\\", tmpPath, ICQ_PROTOCOL_NAME);
        mir_free(tmpPath);
    }
    else
    {
    ...
    }
    {
        ...
        char* szDir = mir_utf8encode(pszDest);
...
void StartAvatarThread(HANDLE hConn, char* cookie, WORD cookieLen) // called from event
{
    ...
    atsi = (avatarthreadstartinfo*)icq_alloc_zero(sizeof(avatarthreadstartinfo));
    atsi->pendingLogin = 1;
    // Randomize sequence
    ...
    if (avatarType != -1)
    {
        ...
        if (bJob)
        {
            ...
            if (bAutoLoad)
            {
                ...
                GetAvatarData(hContact, dwUIN, szUID, (char*)pAvatarHash, cbAvatarHash, szAvatar);
...
{
    avatarrequest *ar = NULL;

    ar = (avatarrequest*)icq_alloc_zero(sizeof(avatarrequest));
    if (ar)
        ar->type = type;
...
int GetAvatarData(HANDLE hContact, DWORD dwUin, char* szUid, char* hash, unsigned int hashlen, char* file)
{
    ...
    EnterCriticalSection(&cookieMutex); // wait for ready queue, reused cs
    {
        ...
        ar->hash = (char*)icq_alloc_zero(hashlen);
...
int SetAvatarData(HANDLE hContact, WORD wRef, char* data, unsigned int datalen)
{
    ...
    EnterCriticalSection(&cookieMutex); // wait for ready queue, reused cs
    {
        ...
        ar->hContact = hContact;
        ar->pData = NULL;
        ar->pData = (char*)icq_alloc_zero(datalen);
        if (!ar->pData)
        {
            // alloc failed
            LeaveCriticalSection(&cookieMutex);
...
static unsigned __stdcall icq_avatarThread(void* pParam)
{
    avatarthreadstartinfo *atsi = (avatarthreadstartinfo *)pParam;
        ...
        while(!atsi->stopThread)
        {
            ...
            if ((AvatarsReady == TRUE) && (packetRecv.bytesAvailable == packetRecv.bytesUsed) && !atsi->paused) // no packets pending
            {
                ...
                while (pendingRequests && atsi->runCount < 3) // pick up an request and send it - happens immediatelly after login
                {
                    ...
                    pendingRequests = (avatarrequest*)reqdata->pNext;
...
void handleAvatarLogin(BYTE *buf, WORD datalen, avatarthreadstartinfo *atsi)
{
    ...
    if (*(DWORD*)buf == 0x1000000)
    {
        ...
        serverCookieInit(&packet, (BYTE*)atsi->pCookie, atsi->wCookieLen);
        sendAvatarPacket(&packet, atsi);
...
void handleAvatarFam(BYTE *pBuffer, WORD wBufferLength, snac_header* pSnacHeader, avatarthreadstartinfo *atsi)
{
    ...
    case ICQ_AVATAR_GET_REPLY:  // received avatar data, store to file
    {
        ...
        if (FindCookie(pSnacHeader->dwRef, NULL, (void**)&ac))
        {
            ...
            WORD datalen;
            int out;
            char* szMyFile = (char*)icq_alloc_zero(strlennull(ac->szFile)+10);
