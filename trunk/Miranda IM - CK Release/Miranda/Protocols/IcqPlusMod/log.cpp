...
static unsigned __stdcall icq_LogMessageThread(void* arg)
{
...
void icq_LogUsingErrorCode(int level, DWORD dwError, const char *szMsg)
{
    ...
    default:
        if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError, 0, szErrorMsg, SIZEOF(szErrorMsg), NULL))
            pszErrorMsg = szErrorMsg;
        else
            pszErrorMsg = "";
        break;
    }
    pszErrorMsgUtf = mir_utf8encode(pszErrorMsg);
