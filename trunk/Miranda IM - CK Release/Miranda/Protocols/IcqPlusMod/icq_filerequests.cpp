...
void handleFileAck(PBYTE buf, WORD wLen, DWORD dwUin, DWORD dwCookie, WORD wStatus, char* pszText)
{
    ...
    if (wFilenameLength > 0)
    {
        ...
        pszFileName = (char*)icq_alloc_zero(wFilenameLength+1);
...
void handleFileRequest(PBYTE buf, WORD wLen, DWORD dwUin, DWORD dwCookie, DWORD dwID1, DWORD dwID2, char* pszDescription, int nVersion, BOOL bDC)
{
...

    {
        ...
        ft->szFilename = mir_utf8encode(pszFileName);
        ft->szDescription = mir_utf8encode(pszDescription);
        ft->fileId = -1;
        ...
        // Send chain event
        szBlob = (char*)icq_alloc_zero(sizeof(DWORD) + strlennull(pszFileName) + strlennull(pszDescription) + 2);
        *(PDWORD)szBlob = 0;
