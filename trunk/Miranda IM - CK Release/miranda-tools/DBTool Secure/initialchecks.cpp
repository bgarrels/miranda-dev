...
int WorkInitialChecks(int firstTime)
{
	...
	EncReadFile(opts.hFile,&dbhdr,sizeof(dbhdr),&bytesRead,NULL);
	...
	//check if it's secured or not
	if(!memcmp(dbhdr.signature,&dbSecSignature,sizeof(dbhdr.signature))){
		if(EncGetPassword(&dbhdr,opts.filename)){
			AddToStatus(STATUS_FATAL,"Given password is incorrect or was not given at all!");
	...
	if(opts.bAggressive) {
		...
		*_tcsrchr( opts.workingFilename, '.' ) = 0;
		...
		for(;;) {
			EncReadFile(opts.hFile,buf,sizeof(buf),&bytesRead,NULL);
			EncWriteFile(hFile,buf,bytesRead,&bytesWritten,NULL);
