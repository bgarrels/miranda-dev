...
rates* ratesCreate(BYTE* pBuffer, WORD wLen)
{
    ...
    pRates = (rates*)icq_alloc_zero(sizeof(rates)+(wCount)*sizeof(rates_group));
    ...
    for (i=0; i<wCount; i++)
    {
        ...
        pGroup->nPairs = wNum;
        pGroup->pPairs = (WORD*)icq_alloc_zero(wNum*4);
        for (n=0; n<wNum*2; n++)
        {
            WORD wItem;
...
void ratesRelease(rates** pRates)
{
    if (pRates)
    {
        ...
        if (rates)
        {
            ...
            {
                mir_free(rates->groups[i].pPairs);
            }
            mir_free(*pRates);
        }
...
static unsigned __stdcall rateDelayThread(void* p)
{
    rate_delay_args* pArgs = (rate_delay_args*)p;
...
{
    rate_delay_args* pArgs = NULL;

#ifdef _DEBUG
    NetLog_Server("Delay %dms", nDelay);
#endif

    pArgs = (rate_delay_args*)icq_alloc_zero(sizeof(rate_delay_args)); // This will be freed in the new thread
...
static void RatesTimer1()
{
    ...
    if (!icqOnline)
    {
        ...
        for (i=0; i<pendingListSize1; i++) mir_free(pendingList1[i]);
        mir_free(&pendingList1);
        pendingListSize1 = 0;
        LeaveCriticalSection(&ratesListsMutex);
...
static void putItemToQueue1(rate_record *item, int nLev)
{
    ...
    if (pendingListSize1)
    {
        ...
        pendingListSize1++;
        pendingList1 = (rate_record**)mir_realloc(pendingList1, pendingListSize1*sizeof(rate_record*));
        tmp = (rate_record*)icq_alloc_zero(sizeof(rate_record));
    ...
    else
    {
        ...
        pendingListSize1++;
        pendingList1 = (rate_record**)icq_alloc_zero(sizeof(rate_record*));
        tmp = (rate_record*)icq_alloc_zero(sizeof(rate_record));
        memcpy(tmp, item, sizeof(rate_record));
        pendingList1[0] = tmp;
...
static void RatesTimer2()
{
    ...
    if (!icqOnline)
    {
        ...
        {
            mir_free(pendingList2[i]->szData);
            mir_free(pendingList2[i]);
        }
        mir_free(&pendingList2);
        pendingListSize2 = 0;
        LeaveCriticalSection(&ratesListsMutex);
...
static void putItemToQueue2(rate_record *item, int nLev)
{
    ...
    if (!bFound)
    {
        ...
        pendingListSize2++;
        pendingList2 = (rate_record**)mir_realloc(pendingList2, pendingListSize2*sizeof(rate_record*));
    }
    tmp = (rate_record*)icq_alloc_zero(sizeof(rate_record));
    memcpy(tmp, item, sizeof(rate_record));
