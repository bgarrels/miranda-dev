...
static VOID CALLBACK CheekySearchTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime)
{
...
INT_PTR IcqBasicSearch_v9(WPARAM wParam, LPARAM lParam)
{
    ...
    int nHandle = 0;
    unsigned int i, j;
    TCHAR *pszSearch = (TCHAR*)lParam;

    if (_tcslen(pszSearch) == 0)
        return 0;


    if (!gbAimEnabled)
    {
        for (i=j=0; (i<_tcslen(pszSearch)) && (j<255); i++)
        {
...
INT_PTR IcqSearchByEmail(WPARAM wParam, LPARAM lParam)
{
    ...
    if(_mirandaVersion > PLUGIN_MAKE_VERSION(0,9,0,7))
    {
        tszMail = (TCHAR*)lParam;
        szMail = mir_t2a(tszMail);
        i = _tcslen((TCHAR*)lParam);
    }
    else
    {
        szMail = (char*)lParam;
    ...
    if (lParam && icqOnline && i > 0)
    {
        DWORD dwSearchId, dwSecId;

        // Success
        dwSearchId = SearchByEmail(szMail);
        if (gbAimEnabled)
            dwSecId = icq_searchAimByEmail(szMail, dwSearchId);
        else
            dwSecId = 0;
//	if(_mirandaVersion > PLUGIN_MAKE_VERSION(0,9,0,7))
//		mir_freeszMail);
...
INT_PTR IcqSearchByDetails(WPARAM wParam, LPARAM lParam)
{
    if (lParam && icqOnline)
    {
        ...
        DWORD dwCookie = 0;
        char *szNick, *szFirstName, *szLastName;
...
#ifndef _08CORE
            if(_mirandaVersion > PLUGIN_MAKE_VERSION(0,9,0,7))
                dwCookie = SearchByNames_v9(psbn->pszNick, psbn->pszFirstName, psbn->pszLastName);
            else
#endif
            {
#ifndef _08CORE
                szNick = mir_u2a(psbn->pszNick);
                szFirstName = mir_u2a(psbn->pszFirstName);
                szLastName = mir_u2a(psbn->pszLastName);
#else
                szNick = psbn->pszNick;
                szFirstName = psbn->pszFirstName;
                szLastName = psbn->pszLastName;
#endif
                dwCookie = SearchByNames(szNick, szFirstName, szLastName, 0);
                mir_free(szNick);
                mir_free(szFirstName);
                mir_free(szLastName);
            }
        }
        if(dwCookie)
...
INT_PTR IcqCreateAdvSearchUI(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        // Success
        return (int)CreateDialogUtf(hInst, MAKEINTRESOURCEA(IDD_ICQADVANCEDSEARCH), (HWND)lParam, AdvancedSearchDlgProc);
    }
    ...
    if (lParam)
    {
...
#ifndef _08CORE
                char szUid[MAX_PATH];
                if(_mirandaVersion >= PLUGIN_MAKE_VERSION(0,9,0,6))
                    if (isr->hdr.flags & PSR_UNICODE)
                    {
                        char *tmp = mir_utf8encodeW((WCHAR*)isr->hdr.id);
                        strcpy(szUid, tmp);
                        mir_free(tmp);
                    }
                    else
        ...
        else
        {
            ...
            if (IsStringUIN(szUid))
                return (int)AddToListByUIN(atoi(szUid), wParam);
            else
                return (int)AddToListByUID(szUid, wParam);
...
INT_PTR IcqGetInfo(WPARAM wParam, LPARAM lParam)
{
    if (lParam && icqOnline)
    {
        ...
        if (dwUin)
            dwCookie = icq_sendGetInfoServ(ccs->hContact, dwUin, (ccs->wParam & SGIF_ONOPEN) != 0);
    ...
    if (lParam && icqOnline)
    {
        ...
        if (dwUin)
        {
            ...
            case ID_STATUS_ONLINE:
            {
                ...
                DBGetContactSettingTString( ccs->hContact, ICQ_PROTOCOL_NAME, "MirVer", &dbv );
        ...
        if (ccs->hContact && ccs->lParam)
        {
                        ...
                        !ICQGetContactSettingByte(ccs->hContact, "UnicodeSend", 1))
                {
                    ...
                    if (!plain_ascii)
                    {
                        ...
                        else if (!pszText)
                        {
                            ...
                            pszText = mir_utf8decodeA((char*)puszText);
                            bNeedFreeA = 1;
                        }
                    }
                    else if (!pszText)
            ...
            else
            {
                ...
                if (!dwUin)
                {
                    ...
                    src = (char*)icq_alloc_zero(strlennull(mng) + 28);
                    ...
                    {
                        if (bNeedFreeA) mir_free(pszText);
                        pszText = src;
                        bNeedFreeA = 1;
                    }
                }
                ...
                if (!dwUin || !CheckContactCapabilities(ccs->hContact, CAPF_SRV_RELAY) || wRecipientStatus == ID_STATUS_OFFLINE)
                {
                    ...
                    if (puszText) pwszText = mir_utf8decodeW((char*)puszText);	// FIXME
        ...
        if (ccs->hContact && ccs->lParam)
        {
            ...
            else
            {
                if (CheckContactCapabilities(ccs->hContact, CAPF_CONTACTS) && wRecipientStatus != ID_STATUS_OFFLINE)
                {
                    ...
                    contacts = (struct icq_contactsend_s*)icq_alloc_zero(sizeof(struct icq_contactsend_s)*nContacts);
                    {
                        nDataLen = 0;
                        nNamesLen = 0;
                        ...
                        if (i == nContacts)
                        {
                            ...
                            mData.wPlace = 0;
                            mData.pData = (BYTE*)icq_alloc_zero(nDataLen);
                            mData.wLen = nDataLen;
                            mNames.wPlace = 0;
                            mNames.pData = (BYTE*)icq_alloc_zero(nNamesLen);
                            ...
                            {
                                // rate is too high, the message will not go thru...
                                mir_free(mData.pData);
                                mir_free(mNames.pData);
                            ...
                            // AIM clients do not send acknowledgment
                            if (!dwUin && pCookieData->nAckType == ACKTYPE_CLIENT)
                                pCookieData->nAckType = ACKTYPE_SERVER;
                            // Send the message
                            dwCookie = icq_SendChannel2Contacts(dwUin, szUid, ccs->hContact, (const char*)mData.pData, mData.wPlace, (const char*)(mNames.pData), mNames.wPlace, pCookieData);	// FIXME
                ...
                else if (dwUin)
                {
                    ...
                    contacts = (struct icq_contactsend_s*)icq_alloc_zero(sizeof(struct icq_contactsend_s)*nContacts);
                    {
                        ...
                        if (i == nContacts)
                        {
                            ...
                            pBuffer = pBody = (char *)icq_alloc_zero(nBodyLength);
...
INT_PTR IcqSendAuthRequest(WPARAM wParam, LPARAM lParam)
{
    if (lParam && icqOnline)
    {
        CCSDATA* ccs = (CCSDATA*)lParam;
        ...
        if (ccs->hContact)
        {
            ...
            if (dwUin && ccs->lParam)
            {
                ...
                utf = mir_utf8encode(text); // Miranda is ANSI only here
...
INT_PTR IcqRecvContacts(WPARAM wParam, LPARAM lParam)
{
    ...
    pBlob = (PBYTE)icq_alloc_zero(cbBlob);
    for (i = 0, pCurBlob = pBlob; i < pre->lParam; i++)
    {
        strcpy((char*)pCurBlob, isrList[i]->hdr.nick);	// FIXME
        pCurBlob += strlennull((const char*)pCurBlob) + 1;
        if (isrList[i]->uin)
        {
            _itoa(isrList[i]->uin, szUin, 10);
            strcpy((char*)pCurBlob, szUin);	// FIXME
        }
        else // aim contact
            strcpy((char*)pCurBlob, isrList[i]->uid);	// FIXME
