			...
			for(std::list<std::wstring>::iterator it = files.begin(); it != files.end(); ++it)
			{
				...
				if(ret == -3)
				{
					...
					else
					{
						...
						for(ret = 0; ret < contacts.size(); ++ret)
						{
...
bool ExecuteCurrentTask(time_t now)
{
	...
	if(isExport)
	{
		...
		if(to.forceExecute)
		{
			...
			{
				TCHAR* name = new TCHAR[to.taskName.size() + 1];
				memcpy(name, to.taskName.c_str(), (to.taskName.size() + 1) * sizeof(TCHAR));
	...
	if (zf != NULL)
	{
		...
		do
		{
			...
			if (err == UNZ_OK)
			{
				...
				for (int i = 0; i < fileNameInZip.length(); ++i) 
				{
...
INT_PTR ExecuteTaskService(WPARAM wParam, LPARAM lParam)
{
	...
	if(taskNr < 0 || taskNr >= Options::instance->taskOptions.size())
	{
		LeaveCriticalSection(&Options::instance->criticalSection);
		return FALSE;
	}
