...
{
	int conv = WideCharToMultiByte(CP_UTF8, 0, str.c_str(), str.length() + 1, NULL, 0, NULL, NULL);
	if(conv > memBuf.size())
	{
		memBuf.resize(conv);
	}

	conv = WideCharToMultiByte(CP_UTF8, 0, str.c_str(), str.length() + 1, (char*)memBuf.c_str(), conv, NULL, NULL);
	return conv;
}
